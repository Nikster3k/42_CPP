...
int	main(void)
{
	...
	Form	document("Divorce Papers", 3, 1);
	...
	Form generic("Generic Document" , 100, 5);
	...
	try
	{
		Form under("Just a little", 151, 1);
	}
	catch(const std::exception& e)
	{
		std::cerr << e.what() << '\n';
	...
	try
	{
		Form under("Just a little", 1, 151);
	}
	catch(const std::exception& e)
	{
		std::cerr << e.what() << '\n';
	...
	try
	{
		Form over("Just a little", -2147483648, 1);
	}
	catch(const std::exception& e)
	{
		std::cerr << e.what() << '\n';
